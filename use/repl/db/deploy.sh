...
if [ "$2" = "rv" ]; then
   docker volume rm 'd-dodeka_repl-db-volume-production' || exit
fi
...
if [ "$( docker container inspect -f '{{.State.Status}}' d-dodeka_repl-db-1 )" = "running" ];
then
    echo "PostgreSQL startup successful."
else
    echo "PostgreSQL startup failed."
    # If fail, check logs
    docker container logs d-dodeka_repl-db-1
